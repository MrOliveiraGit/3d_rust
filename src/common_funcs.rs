...
pub fn get_grid_normals(n: usize, y_vals: &Vec<f32>) -> Vec<f32>{
    ...
    for z in 0..points_per_row{
        for x in 0..points_per_row {
            ...
            if z == n || x == n {
                return_var[return_var_start_index + 1] = 1.;
            }else {
...
pub fn get_updated_3d_y_values(curr_time: f32) ->Vec<f32>{
    let point_counter_per_row = GRID_SIZE + 1;
    let mut y_vals: Vec<f32> = vec![0.;point_counter_per_row * point_counter_per_row];
    let half_grid: f32 = point_counter_per_row as f32 / 2.;
    let frequency_scale: f32 = 3. * std::f32::consts::PI;
    let y_scale = 0.15;
    let sin_offset = curr_time / 1000.;

    for z in 0..point_counter_per_row{
        for x in 0..point_counter_per_row{
            let use_y_index = z * point_counter_per_row + x;
...
pub fn link_program(
    ...
    let program = gl
     .create_program()
     .ok_or_else(|| String::from("error creating progrma"))?;
    ...
    if gl.get_program_parameter(&program,WebGlRenderingContext::LINK_STATUS)
     ...
     .unwrap_or(false){
         ...
         Err( 
             gl.get_program_info_log(&program).unwrap_or_else(|| String::from("Unknown error creating program object"))
         )
     }
}
...
pub fn get_position_grad_n_by_n(n: usize) -> (Vec<f32>,Vec<u16>){
    let n_plus_one = n + 1;
